...
class ProjectsSection extends HTMLElement {
  connectedCallback() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .scroll-animate.fade-in-up { opacity: 0; transform: translateY(40px); }
        .scroll-animate.scale-in { opacity: 0; transform: scale(0.95); }
        ...
        .project-pill.scroll-animate.fade-in-up { transform: translateY(40px); }
        .project-pill.scroll-animate.stagger-1 { transition-delay: 0.1s; }
        .project-pill.scroll-animate.stagger-2 { transition-delay: 0.2s; }
        .project-pill.scroll-animate.stagger-3 { transition-delay: 0.3s; }
        .project-pill.scroll-animate.stagger-4 { transition-delay: 0.4s; }
        ...
        /* --- LAYOUT WRAPPER (Side Arrow + Panel) --- */
        .feature-wrapper {
          display: flex;
          align-items: center;
          gap: 2rem;
        ...
        button.side-chevron:hover {
          color: #04d7c2;
          transform: translateY(2px);
          filter: drop-shadow(0 0 8px rgba(4, 215, 194, 0.6));
        }

        button.side-chevron svg {
          width: 32px;
          height: 32px;
        ...
        /* RESTORED: Hover Glow for Industry/Region Tags */
        .tag:hover {
          background: linear-gradient(120deg, #04d7c2, #00b9ff);
          color: #021019;
          border-color: transparent;
        ...
        /* RESTORED: Hover Glow for Tech Chips */
        .tech-chip:hover {
          background: linear-gradient(120deg, #04d7c2, #00b9ff);
          color: #021019;
          border-color: transparent;
        ...
        .slider-arrow:hover {
          border-color: #04d7c2;
          color: #04d7c2;
          box-shadow: 0 0 12px rgba(4, 215, 194, 0.3);
        ...
        .slider-dash.is-active {
          width: 40px;
          background: #04d7c2;
          box-shadow: 0 0 8px rgba(4, 215, 194, 0.5);
        }
        ...
        .project-pill span { display: block; }
        ...
        /* RESTORED: Hover Glow for Grid Chips */
        .pill-tech-chip:hover {
          background: linear-gradient(120deg, #04d7c2, #00b9ff);
          color: #021019;
          border-color: transparent;
        ...
        .project-pill:hover {
          transform: translateY(-4px);
          border-color: rgba(4, 215, 194, 0.8);
          box-shadow: 0 0 20px rgba(4, 215, 194, 0.4), 0 0 40px rgba(4, 215, 194, 0.3);
        }

        .project-pill.is-active {
          border-color: rgba(56, 189, 248, 0.95);
          box-shadow: 0 0 0 1px rgba(15, 23, 42, 1), 0 0 0 2px rgba(56, 189, 248, 0.75);
          background: radial-gradient(circle at top left, rgba(8, 47, 73, 0.95), rgba(15, 23, 42, 0.98));
          transform: scale(1.02);
        }
      ...
      <div class="container">
        ...
        <div class="feature-wrapper">
          ...
          <div class="side-nav-container">
            <button class="side-chevron" aria-label="Next project">
        ...
        <div class="slider-controls">
          ...
          <div class="slider-indicators" data-dots>
            </div>
  ...
  _initAnimations() {
    ...
    const projectPills = root.querySelectorAll('.project-pill');
    const ctaBtn = root.querySelector('.project-cta');
    const sliderControls = root.querySelector('.slider-controls');
    
    if (header) header.classList.add('scroll-animate', 'fade-in-up');
    if (featurePanel) featurePanel.classList.add('scroll-animate', 'fade-in-up');
    if (sliderControls) sliderControls.classList.add('scroll-animate', 'fade-in-up');
    if (ctaBtn) ctaBtn.classList.add('scroll-animate', 'scale-in');
    ...
    root.querySelectorAll('.scroll-animate').forEach(el => observer.observe(el));
    observer.observe(this);
  }
  ...
  _cacheElements() {
    ...
    this.sideChevron = root.querySelector(".side-chevron");
  }

  _buildControls() {
    // Generate Dashes (Bottom Slider)
    this.dotsContainer.innerHTML = "";
    this.projects.forEach((_, index) => {
      const dot = document.createElement("button");
      dot.type = "button";
      dot.className = "slider-dash";
      dot.ariaLabel = `Go to project ${index + 1}`;
      dot.addEventListener("click", () => this._showProject(index));
      this.dotsContainer.appendChild(dot);
    });
    this.dotButtons = Array.from(this.dotsContainer.children);

    // Attach Bottom Arrow Listeners
    if(this.prevBtn) {
      this.prevBtn.addEventListener("click", () => {
        const newIndex = (this.currentIndex - 1 + this.projects.length) % this.projects.length;
        this._showProject(newIndex);
      });
    }
    ...
    // Attach Side Chevron Listener
    if (this.sideChevron) {
      this.sideChevron.addEventListener("click", () => {
        const newIndex = (this.currentIndex + 1) % this.projects.length;
        this._showProject(newIndex);
      });
    }
